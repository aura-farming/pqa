...
from pqa.memory import Failure, connect, record_failure, record_precipitate, recent_failures
...
def test_record_and_read_precipitate(conn: sqlite3.Connection):
    record_precipitate(conn, "s1", "rate limiter", "token-bucket-stream", "beat the queue on bursts")
