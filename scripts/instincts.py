...
def export(db: str, out: str) -> int:
    conn = _conn(db)
    rows = conn.execute(
        "SELECT name, statement, confidence, evidence_n, origin FROM instincts ORDER BY confidence DESC"
    ).fetchall()
    conn.close()
    payload = {"format": "pqa-instincts/1", "exported_at": int(time.time()),
               "instincts": [dict(zip(("name", "statement", "confidence", "evidence_n", "origin"), r)) for r in rows]}
...
def import_(db: str, src: str) -> int:
    ...
    for it in data.get("instincts", []):
        existing = conn.execute("SELECT confidence FROM instincts WHERE name=?", (it["name"],)).fetchone()
        ...
        conn.execute(
            ...
            (it["name"], it["statement"], float(it.get("confidence", 0.5)),
             int(it.get("evidence_n", 0)), tag, int(time.time())),
        )
        added += 1
